...
| `VITE_SUPABASE_URL` | Optional. Project URL (`https://xxxx.supabase.co`) so the production CSP `img-src` can pin signed avatar URLs. Unset → `https://*.supabase.co`. Not used by app JS — avatars still load via the backend. |
