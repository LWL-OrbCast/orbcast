...
**API-Sports `finished` is not HIP-4 `settled`.** `status.short` `FT` / `AET` / `PEN` / `AWD` / `WO` means the match is over. The Outcome / Hyperliquid book stays `open` until the venue calls `settleQuestion2` (event markets, validator vote — Outcome targets resolution within ~4 hours, and the contest template fixes the official result one hour after full-time). Home, Markets, search, featured, and trending hide football **contest** books that match a finished overlay fixture, **or** whose kickoff is already in the past and the overlay no longer lists them as live/upcoming. HIP-4 `live` and a held position do **not** keep an FT book in All or Football — Positions is enough. A direct `/market/:id` stays reachable. We do not invent settlement.
