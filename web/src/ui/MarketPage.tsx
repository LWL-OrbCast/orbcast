...
export function MarketPage() {
  ...
  const limitCents = waitPx != null ? Math.round(waitPx * 100) : null;
      ...
      <aside className="h-fit rounded-2xl border border-[var(--border)] bg-white p-5 lg:col-start-2 lg:row-start-1 lg:row-span-2 lg:sticky lg:top-20">
        {selectedMarket.status === 'settled' ? (
          ...
          <>
            ...
            <p className="mt-3 text-xs text-[var(--text-3)]">{hip4.ticket.feeHint}</p>
            ...
            {limit ? (
              <div className="relative mt-2">
                <input
                  ...
                  className="w-full rounded-xl border border-[var(--border)] px-3 py-2 pr-8 text-sm"
                />
            ...
            {authenticated && liveSetup && !liveSetup.agent ? (
              ...
              <p className="mt-2 text-xs text-[var(--text-2)]">{hip4.wallet.firstOrderFeeHint}</p>
