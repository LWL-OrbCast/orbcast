...
export function RewardsSkeleton({ compact }: { compact?: boolean }) {
  return (
    <div aria-busy="true">
      {compact ? null : (
        <>
      ...
      <div
        className={`${compact ? '' : 'mt-4'} grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,380px)] lg:gap-6`}
      >
        <div className="min-w-0">
          ...
          <div className="mt-4 space-y-2">
            {Array.from({ length: 4 }, (_, i) => (
              <Skel key={i} className="h-14 w-full rounded-xl" />
            ))}
          </div>
        </div>
...
export function AuthGate({
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setHydrateTimedOut(true), 6000);
    return () => window.clearTimeout(t);
  }, [hydrating]);
  if (hydrating && !hydrateTimedOut) return skeleton;
